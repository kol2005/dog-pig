...
-- 1. 전체 칼럼에 모든 데이터를 다 입력하고 데이터를 추가할때만 가능
...
SELECT 
    DECODE(io_inout,'매입',io_total,0) AS 매입,
